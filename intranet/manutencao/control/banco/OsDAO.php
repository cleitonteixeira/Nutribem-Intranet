...
if(isset($_POST['OS']) && $_POST['OS']  ==  "Cadastrar"){
    ...
    try{
        ...
        $sql  = 'SELECT u.*, (SELECT nOS FROM os WHERE idOS = ?) AS nOS FROM usuarios u WHERE idusuarios = (SELECT Responsavel_idResponsavel FROM os WHERE idOs = ?);'; 
        $stmt = $conexao->prepare($sql);
        $stmt->bindParam(1, $idOs);
        ...
        <html>
            ...
            <body>
                <table width='100%' border='0' cellspacing='0' cellpadding='20 background='".BASE."img/Fundo.png'>
                    <tr>
                        <td>
                            <p>OS: <strong>".$rest->nOS."</strong> agendada para o dia ".date("d/m/Y", strtotime($DataAg))." por <strong>".$_SESSION['Nome']."</strong></p>
                            <br />
                            <p>Este e-mail foi enviado em <b> $data_envio </b> às <b> $hora_envio </b></p>
                        </td>
                    </tr>
        ...
        $email->From        = 'contato@nutribemrefeicoescoletivas.com.br';
        $email->FromName    = 'Manutencao On-line';
        $email->Subject     = 'Manutencao On-line: OS Agendada '.$rest->nOS;
        $email->IsHTML(true); // Define que o e-mail será enviado como HTML
        $email->Body        = $MsgEmail;
    ...
    try{
        ...
        <html>
            ...
            <body>
                <table width='100%' border='0' cellspacing='0' cellpadding='20 background='".BASE."img/Fundo.png'>
                    <tr>
                        <td>
                            <p>Novo comentário na OS: ".$rest->nOS." feito por <strong>".$_SESSION['Nome']."</strong></p>
                            <br />
                            <p>Este e-mail foi enviado em <b> $data_envio </b> às <b> $hora_envio </b></p>
                        </td>
                    </tr>
        ...
        $email->FromName    = 'Manutencao On-line';
        if(!isset($_POST['BTFinaliza'])){
            $email->Subject     = 'Manutencao On-line: Novo comentario na OS: '.$rest->nOS;
        }else{
            $email->Subject     = 'Manutencao On-line: OS: '.$rest->nOS. 'FINALIZADA';
        }
        $email->IsHTML(true); // Define que o e-mail será enviado como HTML
        $email->Body        = $MsgEmail;
