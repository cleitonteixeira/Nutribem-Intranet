...
function LoadItem($un){
    ...
    $sql = "SELECT c.Nome AS Equipamento, e.*, ce.*, cdu.Nome AS Unidade FROM equipamento e INNER JOIN cadastro c ON idCadastro = Cadastro_idCadastro INNER JOIN categoriaequipamento ce ON idCategoriaEquipamento = Categoria_idCategoria INNER JOIN unidademt umt ON umt.idUnidadeMT = e.Unidade_idUnidade INNER JOIN cadastro cdu ON cdu.idCadastro = umt.Cadastro_idCadastro WHERE umt.idUnidadeMT = ?;";
