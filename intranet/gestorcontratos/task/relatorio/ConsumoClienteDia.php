    ...
    <div class="row">
        <div class="col-xs-12 col-md-12 col-lg-12 conteudo">
            <div class="col-xs-12 col-lg-12 col-md-12">
                <h1 class="text-center">RELATÓRIO DE CONSUMO POR UNIDADE/CLIENTE POR PERIODO</h1>
            </div>
            <div class="conteudo"></div>
            <form  name="Form" role="form" action="RelConsumoClienteDia.php" target="_blank" method="post" enctype="multipart/form-data" class="form-horizontal text-center" data-toggle="validator" id="FormCliente1" name="FormCliente1" >
                <div class="form-group">
                    ...
                    <div class="col-sm-7">
                        <select class="selectpicker form-control dropdown" required name="Unidade" id="Unidade" title="Selecione um Unidade" data-size="5" data-live-search="true" <?php echo isset($_POST['Diario']) && $_POST['Diario'] == "Data-Unidade" ? "disabled" : ''; ?> >
                            <?php
                            $sql = 'SELECT ca.CNPJ, ca.Nome AS Empresa FROM empresa em INNER JOIN cadastro ca ON ca.idCadastro = em.Cadastro_idCadastro';
                            $stm = $conexao->prepare($sql);
                            $stm->execute();
                            $rs = $stm->fetchAll(PDO::FETCH_OBJ);
                            foreach($rs as $r):
                            ?>
                            <optgroup label="<?php echo CNPJ_Padrao($r->CNPJ)." - ".utf8_decode($r->Empresa); ?>" >
                                <?php
