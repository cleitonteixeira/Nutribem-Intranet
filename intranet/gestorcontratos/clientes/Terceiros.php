...
<script>
	$(document).ready(function(){
        ...
        //$('#gerar-contrato').hide();
        $("select[name='Terceiro']").change(function(e){
            var ct = $('#Terceiro').val();
            if($('#proposta').val() != ""){
                var prop = $('#proposta').val();
                document.getElementById('gerar-contrato').href="<?php echo BASE; ?>clientes/Contrato.php?p="+prop+"&ct="+ct;
                //$('#gerar-contrato').show();
            }else{
                $('Terceiro').val('').change();   
            }
	...
	<div class="conteudo">
        ...
        <form  name="Form" role="form" action="" method="post" enctype="multipart/form-data" class="form-horizontal text-center" data-toggle="validator" id="FormCliente" name="FormCliente" >
            <div class="form-group">
                ...
                <div class="col-sm-6">
                    <select class="selectpicker form-control dropdown" name="cliente" id="cliente" title="Selecione um Cliente Principal" data-size="5" data-header="Selecione um Cliente" data-live-search="true" autofocus required>
                        <?php
                        foreach($rs as $r ){
                        ?>
            ...
            <div class="form-group">
             ...
             <div class="form-group">
                <label class="control-label col-sm-2" for="Terceiro">Terceiro: </label>
                <div class="col-sm-6">
                    <select class="selectpicker form-control dropdown" name="Terceiro" id="Terceiro" title="Selecione um Cliente" data-size="5" data-header="Selecione um Cliente" data-live-search="true" required>
                        <?php
                        foreach($rs as $r ){
                        ?>
                        <option data-tokens="<?php echo $r->CNPJ.' '.utf8_decode($r->Cliente).' '.str_pad($r->idContratante,3,0,STR_PAD_LEFT); ?>" data-subtext="CNPJ: <?php echo CNPJ_Padrao(str_pad($r->CNPJ,14,0,STR_PAD_LEFT)); ?>" value="<?php echo $r->idContratante ?>" ><?php echo str_pad($r->idContratante,3,0,STR_PAD_LEFT)." - ".utf8_decode($r->Cliente); ?></option>
