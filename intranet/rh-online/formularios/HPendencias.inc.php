...
function getSolicitanteU($cod){
    ...
    $sql = "SELECT DISTINCT(p.Usuario_idUsuario) AS idSolicitante, u.Nome AS Solicitante FROM pendencias p INNER JOIN usuarios u ON u.idusuarios = p.Usuario_idUsuario WHERE Unidade_idUnidade ".$var." AND Resultado IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)') ORDER BY Data DESC;";
...
function getTipoU($cod){
    ...
    $sql = "SELECT DISTINCT(p.Tipo) AS Tipo FROM pendencias p INNER JOIN usuarios u ON u.idusuarios = p.Usuario_idUsuario WHERE p.Unidade_idUnidade ".$var." AND p.Resultado IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)') ORDER BY Data DESC;";
    $stm = $conexao->prepare($sql);
    $stm->bindParam(1, $var_c);
    $stm->execute();
    $Dados = array();
...
function getTipoS($cod){
    ...
    $sql = "SELECT DISTINCT(p.Tipo) AS Tipo FROM pendencias p INNER JOIN usuarios u ON u.idusuarios = p.Usuario_idUsuario WHERE ".$var." AND Resultado IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)') ORDER BY Data DESC;";
    $stm = $conexao->prepare($sql);
    $stm->bindParam(1, $var_c);
    $stm->execute();
    $Dados = array();
...
function getUnidadeS($cod){
    ...
    $sql = "SELECT DISTINCT(p.Unidade_idUnidade) AS idUnidade, c.Nome AS Unidade  FROM pendencias p INNER JOIN unidade u ON u.idUnidade = p.Unidade_idUnidade INNER JOIN cadastro c ON c.idCadastro = u.Cadastro_idCadastro WHERE ".$var." AND Resultado IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)') ORDER BY Data DESC;";
    $stm = $conexao->prepare($sql);
    $stm->bindParam(1, $var_c);
    $stm->execute();
    $Dados = array();
...
function getSolicitanteT($cod){
    ...
    $sql = "SELECT DISTINCT(p.Usuario_idUsuario) AS idSolicitante, u.Nome AS Solicitante FROM pendencias p INNER JOIN usuarios u ON u.idusuarios = p.Usuario_idUsuario WHERE ".$var." AND Resultado IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)') ORDER BY Data DESC;";
    $stm = $conexao->prepare($sql);
    $stm->bindParam(1, $var_c);
    $stm->execute();
    $Dados = array();
...
function getFiltroCompleto($param1,$param2,$param3){
    ...
    $sql = "SELECT * FROM pendencias p WHERE ".$var." AND p.Resultado IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)') ORDER BY p.Data DESC;";
    $stm = $conexao->prepare($sql);
    if($param1 > 0 && $param2 > 0 && $param3 != ""){
