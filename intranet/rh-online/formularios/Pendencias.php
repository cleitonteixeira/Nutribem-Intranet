...
if(in_array($_SESSION['idusuarios'], $a)){
    ...
    if(in_array($_SESSION['idusuarios'], $rh)){
        $sql = "SELECT * FROM pendencias WHERE Resultado NOT IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)')  ORDER BY Data DESC;";
    }else{
        $sql = "SELECT * FROM pendencias WHERE Responsavel_Colaborador IN (".$chefia.") AND Resultado NOT IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)')  ORDER BY Data DESC;";
    }
...
<script>
    $(document).ready(function(){
        $("select[name='unidade']").change(function(e){
            ...
            $.post('Pendencias.inc.php',{ pUnidade: unidade, pUsuario: usuario, pTipo: tipo, FiltroCompleto: 1} , function (dados){
                //alert(dados);
				...
				<div class="col-xs-12 col-lg-12 col-md-12">
                    <form class="form-inline">
                        <div class="form-group">
                            ...
                            <select name="unidade" id="unidade" class="form-control selectpiker">
                        ...
                        <div class="form-group">
                            ...
                            <select name="tipo" id="tipo" class="form-control selectpiker">
                                <option value="" title="Selecione um Tipo">Todos os Tipos</option>
                                <?php
                                $msql = "SELECT DISTINCT(Tipo) FROM pendencias;";
                                $stt = $conexao->prepare($msql);
                                $stt->execute();
						...
						<table class="table table-striped table-bordered table-responsive text-center" name="colaboradores" id="colaboradores">
							...
							<tbody id="pendencias">
								...
								<tr>
									...
									<td><a href="<?php echo BASE;?>formularios/v<?=$rs->Tipo?>.php?p=<?php echo $rs->idPendencias; ?>"><i class="fa fa-external-link-square" aria-hidden="true"></i></a></td>
								</tr>
								<?php
									endwhile;
								?>
