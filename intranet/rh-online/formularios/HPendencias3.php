...
if (!isset($_SESSION['idusuarios'])){
    ...
    if(in_array($_SESSION['idusuarios'], $rh)){
        $sql = "SELECT * FROM pendencias WHERE Resultado IN ('Recusada', 'Validada', 'Recusada (Preenchimento Incorreto)') AND Data >= CURDATE() - INTERVAL 90 DAY ORDER BY idPendencias DESC;";
    }else{
        $sql = "SELECT * FROM pendencias WHERE Responsavel_Colaborador IN (".$_SESSION['boss'].") AND Resultado IN ('Recusada', 'Validada', 'Recusada') AND Data >= CURDATE() - INTERVAL 90 DAY ORDER BY idPendencias DESC;";
    }
