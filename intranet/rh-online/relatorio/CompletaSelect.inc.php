<?php
	...
	function getColaborador($u){
        $conexao = conexao::getInstance();
        $sql = 'SELECT c.CodColaborador AS Codigo, ca.Nome AS Colaborador FROM colaborador c INNER JOIN contratacao ct ON ct.Unidade_idUnidade = ? INNER JOIN cadastro ca ON ca.idCadastro = c.Cadastro_idCadastro WHERE c.Contratacao_idContratacao = ct.idContratacao;';
        $stm = $conexao->prepare($sql);
        $stm->bindValue(1, $u);
        $stm->execute();
        $Dados = array();
        while($row = $stm->fetch(PDO::FETCH_OBJ)){
            $Itens = [
                ...
                "Colaborador" => utf8_decode($row->Colaborador)
            ];
            array_push($Dados, $Itens);
        }
        echo json_encode($Dados);
	...
	function getDadosCargo($u){
        $conexao = conexao::getInstance();
        $sql = 'SELECT cargo, cbo, funcao, salario FROM cargo WHERE idCargo = ?;';
        $stm = $conexao->prepare($sql);
        $stm->bindValue(1, $u);
        $stm->execute();
        $Dados = array();
	...
	function getUnidadeTrans($t){
        $conexao = conexao::getInstance();
        $sql = 'SELECT c.Nome, u.idUnidade FROM unidade u INNER JOIN cadastro c ON c.idCadastro = u.Cadastro_idCadastro WHERE u.idUnidade <> ? ORDER BY u.idUnidade;';
        $stm = $conexao->prepare($sql);
        $stm->bindValue(1, $t);
        $stm->execute();
        $Dados = array();
        while($row = $stm->fetch(PDO::FETCH_OBJ)){
            $Itens = [
                "Codigo" => $row->idUnidade,
                "Unidade" => str_pad($row->idUnidade,2,0,STR_PAD_LEFT)." - ".utf8_decode($row->Nome)
            ];
            array_push($Dados, $Itens);
        }
        echo json_encode($Dados);
    ...
    if(isset($_GET['empresa'])){
        ...
        getColaborador($valor);
    }elseif(isset($_GET['c'])){
        $valor = isset( $_GET['c'] ) ? (int)$_GET['c'] : 0;
        getDadosCargo($valor);
    }elseif(isset($_GET['t'])){
        $valor = isset( $_GET['t'] ) ? (int)$_GET['t'] : 0;
        getUnidadeTrans($valor);
    }
